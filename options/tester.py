...
data = {'Type': ['fm', 'fm', 'fm', 'fm', 'hh', 'hh', 'hh'], 
        'Name': ['theta', 'nu', 'phi_l', 'phi_u', 'alpha', 'shape', 'scale'],
        'Bounds': [(0,1), (0.05, 0.5), (0.05, 0.5), (0.5, 1.5), (0,1), (0,5), (0,5)]} 
...
epochs = 250
I =  11 # input size
O =  df_para.shape[0] # output size = nr of summary statistics
...
train_loader, test_loader = create_dataloader(model_runs=7, model_steps=100, batch_size=24, load=True)
...
def get_theta(y):
  thetas = []
  for i in range(100):
    theta_pred = surrogate(y).cpu().data.numpy()
    thetas.append(theta_pred)
  return thetas


m = Model()
m.run_simulation(100)
y = m.datacollector.get_calibration_data()
y = torch.tensor(y, dtype=torch.float32)

estimated_thetas = get_theta(y)
plot_distributions(estimated_thetas, true_thetas)
print(f'true theta: {true_thetas}')
print(f'estimated theta: {torch.mean(estimated_thetas)}')
