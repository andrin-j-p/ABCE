...
# get simulated data
df_sm_sim, df_hh_sim, df_fm_sim, df_md_sim, df_td_sim = model.datacollector.get_calibration_data()
...
plot_dist(df_fm_sim['stock'].values, 'fm stock', (-300, 2000))
plot_dist(df_hh_sim['income'].values, 'hh income', (-50, 300))
plot_dist(df_hh_sim[df_hh_sim['firm'].notna()] ['income'], 'hh income firm owners',  (-50, 300))
plot_dist(df_hh_sim['money'].values, 'hh money', (-50, 1000))
plot_dist(df_hh_sim[df_hh_sim['firm'].notna()] ['money'], 'hh money firm owners',  (-50, 1000))


#==========================================================================
print(f"true vs. simulated income mean: { np.mean(df_hh_sim['income'])}")
print(f"true vs. simulated income median: { np.median(df_hh_sim['income'])}")
print(f"true vs simulated consumption mean: { np.mean(df_hh_sim['demand'].values)}")
print(f"true vs simulated consumption median: { np.median(df_hh_sim['demand'].values)}")
print(f"total income: {np.sum(df_hh_sim['income'])}")
print(f"total demand: {np.sum(df_hh_sim['demand'])}")
print(f"total money: {np.sum(df_hh_sim['money'])}")
print(f"mean profit: {np.mean(df_fm_sim['profit'])}")
...
print(f"total negative assets: {sum(row.assets for row in df_fm_sim.itertuples()  if row.assets < 0)}")
...
print(df_md_sim.tail(100))

#%%
print(pd.DataFrame([row for row in df_fm_sim.itertuples() if row.assets < 0 ]))
print(f" negative profit {len(pd.DataFrame([row for row in df_fm_sim.itertuples() if row.profit <= 0]))} ")
