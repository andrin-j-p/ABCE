...
class Validation_collector():
  ...
  def collect_data(self):
    ...
    """
    # Trade data is not used and thus emptied to save memory
    amount = pd.DataFrame(self.td_data)['amount']
    self.td_data = []
...
### FM data
    fm_data = [(firm.assets, firm.profit, firm.revenue, firm.stock, firm.price, len(firm.employees), firm.village.treated, firm.market.saturation, firm.market.unique_id) 
                 for firm in self.model.all_firms]
    ...
    self.fm_df = pd.DataFrame(fm_data, columns=['Assets', 'Profit', 'Revenue', 'Stock', 'Price', 'Employees', 'Treated', 'Saturation', 'Market'])
    ...
    df = {'step': self.count, 
          ...
          'Demand_Satisfied': amount.sum() / (self.hh_df['Demand'].sum()/7),
          'Unemployment': len(self.hh_df[self.hh_df['Employer'].isna()]) / self.hh_df.shape[0],
          }
